...
def kin_solver_model_fn(features,labels,mode,params):
    ...
    net = tf.layers.dense(net, units=params['num_joints'],activation=tf.sigmoid)

    # output layer
    out_layer = tf.layers.dense(net,units=params['num_joints'],activation=None)

    result = tf.multiply(out_layer,tf.constant(2*np.pi,dtype=tf.float32))
...
def kin_serving_input_receiver_fn():
    feature_spec = {
        "goal_x":tf.constant([[1]],tf.float32,name="goal_x"),
        "goal_y":tf.constant([[1]],tf.float32,name="goal_y"),
        "goal_z":tf.constant([[1]],tf.float32,name="goal_z"),
    }
